...
class PerimeterController extends Controller
{
	...
	public function getCountPerimeter($id){
		$data = array();
		$protokol = Perimeter::join('master_region','master_region.mr_id','master_perimeter.mpm_mr_id')
					->where('master_region.mr_mc_id',$id)	
					->count();
				
			$data[] = array(
					"jml_perimeter" => $protokol,
	...
	public function uploadProtokol(Request $request)
    {
		...
		if ($request->file_protokol != null || $request->file_protokol != '') {
						...
						$request->file_protokol->move(storage_path() . '/public/protokol/', $name);
    ...
    {
		$this->validate($request, [
            'file_protokol' => 'required|base64mimes:pdf',
			'kd_perusahaan' => 'required',
			'protokol' => 'required',
			'user_id' => 'required',
        ]);
