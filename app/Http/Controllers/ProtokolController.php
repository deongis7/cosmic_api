...
class ProtokolController extends Controller
{
	...
	public function protokol($id){
		$data = array();
		$protokol = Protokol::select('master_protokol.mpt_id','master_protokol.mpt_name','table_protokol.tbpt_mpt_id','table_protokol.tbpt_filename',DB::raw("case when table_protokol.tbpt_date_update::timestamp is not null then table_protokol.tbpt_date_update else table_protokol.tbpt_date_insert end"))
	...
	public function uploadProtokol(Request $request)
    {
		...
		if ($request->file_protokol != null || $request->file_protokol != '') {
					...
					//
		$dataProtokol= TblProtokol::updateOrCreate(['tbpt_mpt_id' => $protokol, 'tbpt_mc_id' => $kd_perusahaan],['tbpt_filename' => $filename_protokol, 'tbpt_user_insert' => $user_id, 'tbpt_user_update' => $user_id])->save();
